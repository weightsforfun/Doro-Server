...
public class UserLectureApi {
    ...
    @GetMapping("/users/{id}")
    public SuccessResponse findMyLectures(@PathVariable("id") Long id,@AuthenticationPrincipal User user) {
        if(user.getId().equals(id) || user.getRole().equals(UserRole.ROLE_ADMIN)) {
